...
std::vector<float> bc_gpu(graph g, int max_threads_per_block, int number_of_SMs, program_options op,
                          const std::set<int> &source_vertices, bool one_deg_reduce, int *weight) {
    //Host result data
    ...
    //Device pointers
    float *bc_d, *delta_d;
    int *d_d, *R_d, *C_d, *F_d, *Q_d, *Q2_d, *S_d, *endpoints_d, *next_source_d, *d_weight;
    ...
    checkCudaErrors(cudaMalloc((void **) &d_weight, sizeof(int) * g.n));
    ...
    checkCudaErrors(cudaMallocPitch((void **) &Q_d, &pitch_Q, sizeof(int) * g.n,
                                    dimGrid.x)); //Making Queues/Stack of size O(n) since we won't duplicate
    ...
    checkCudaErrors(cudaMemcpy(d_weight, weight, sizeof(int) * g.n, cudaMemcpyHostToDevice));
    ...
    if (op.approx) {
        ...
        bc_gpu_opt << < dimGrid, dimBlock >> >
                                 (bc_d, R_d, C_d, F_d, g.n, g.m, d_d, sigma_d, delta_d, Q_d, Q2_d, S_d, endpoints_d, next_source_d, pitch_d, pitch_sigma, pitch_delta, pitch_Q, pitch_Q2, pitch_S, pitch_endpoints, 0, op.k, jia_d, diameters_d, thrust::raw_pointer_cast(
                                         source_vertices_d.data()), true, one_deg_reduce, d_weight);
        checkCudaErrors(cudaPeekAtLastError());
    } else {
        bc_gpu_opt << < dimGrid, dimBlock >> >
                                 (bc_d, R_d, C_d, F_d, g.n, g.m, d_d, sigma_d, delta_d, Q_d, Q2_d, S_d, endpoints_d, next_source_d, pitch_d, pitch_sigma, pitch_delta, pitch_Q, pitch_Q2, pitch_S, pitch_endpoints, 0, g.n, jia_d, diameters_d, thrust::raw_pointer_cast(
                                         source_vertices_d.data()), false, one_deg_reduce, d_weight);
        checkCudaErrors(cudaPeekAtLastError());
    }
...
__global__ void bc_gpu_opt(float *bc, const int *R, const int *C, const int *F, const int n, const int m, int *d,
                           unsigned long long *sigma, float *delta, int *Q, int *Q2, int *S, int *endpoints,
                           int *next_source, size_t pitch_d, size_t pitch_sigma, size_t pitch_delta, size_t pitch_Q,
                           size_t pitch_Q2, size_t pitch_S, size_t pitch_endpoints, int start, int end, int *jia,
                           int *diameters, int *source_vertices, bool approx, bool one_deg_reduce, int *weight) {
    __shared__ int ind;
    __shared__ int i;
    int j = threadIdx.x;
...
//                int stack_iter_len = endpoints_row[current_depth + 1] - endpoints_row[current_depth];
                for (int kk = threadIdx.x + endpoints_row[current_depth];
                     kk < endpoints_row[current_depth + 1]; kk += blockDim.x) {
                    int w = S_row[kk];
                    float dsw = 0;
            ...
            for (int kk = threadIdx.x; kk < n; kk += blockDim.x) {
                atomicAdd(&bc[kk],
                          delta_row[kk]*weight[i]); //Would need to check that kk != i here, but delta_row[kk] is guaranteed to be 0.
            }
        }
        if (j == 0) {
        ...
        if (ind == 2 *
                   DIAMETER_SAMPLES) //Might want to play around with this number. Safe to assume that they are done by now? Probably...
        {
            *jia = 0;
        }
        __syncthreads();
